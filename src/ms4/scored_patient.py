class ScoredPatient:
    def __init__(
        ...
        percentage: float,
        score: float,
        categories: list[tuple[str, str, str, str, float, bool]],
    ) -> None:
    ...
    def generate_json(self) -> str:
        ...
        for category in self.categories:
            ...
            criteria_results_text += (
                '\t\t\t\t\t"description": "' + str(category[1]) + '"\n'
            )
            criteria_results_text += (
                '\t\t\t\t\t"raw_text": "' + str(category[2]) + '"\n'
            )
        ...
        json += (
            criteria_results_text[0:-2] if criteria_results_text else ""
        ) + "\n\t\t\t]\n\t\t}"
